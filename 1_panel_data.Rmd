...
LHS var for individual i at time t is that
$$y_{it}, \ \ \  i = 1,...N, and t = 1,...,T.$$
```{r}
set.seed(6262)
...
$${\bar{x}}_{it}=\left(x_{it1},\ \ldots,\ x_{itK}\right)^\prime,$$
...
$${\bar{X}}_i=\left(\begin{matrix}x_{i11}&\cdots&x_{i1T}\\\vdots&\ddots&\vdots\\x_{iK1}&\cdots&x_{iKT}\\\end{matrix}\right)^\prime,$$
```{r}
#xi
     #x1
...
Using matrix notation for once more, we have
$${\bar{X}}=\left(x_{1},\ \ldots,\ x_{N}\right)^\prime,$$
```{r}
#x
...
also, we have
$$\bar{\beta}=\left(\beta_1,\ \ldots,\ \beta_k\right)$$ 
